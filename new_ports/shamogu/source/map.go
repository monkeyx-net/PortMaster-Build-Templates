...
// TerrainName returns the name of the terrain.
...
// TerrainDesc returns a description of the terrain.
...
// Connected reports whether the given position is connected to an adjacent
// passable tile.
func (m *Map) Connected(p gruid.Point) bool {
...
// AdjacentNonPassableCount returns the number of non passable tiles the player
// is adjacent to, including adjacent tiles outside the map.
func (m *Map) AdjacentNonPassableCount(p gruid.Point) int {
	n := 0
...
// TerrainRuneAt returns the character rune at a given position.
func (m *Map) TerrainRuneAt(p gruid.Point, t rl.Cell) rune {
	if t == Wall {
		for q := range Neighbors(p) {
	...
	return TerrainRune(t)
}

// TerrainRune returns the character rune representing a given terrain.
func TerrainRune(t rl.Cell) (r rune) {
	switch t {
	case Wall:
		r = '#'
	case Floor:
...
// KnownTerrainRuneAt returns the character rune at a given position.
func (m *Map) KnownTerrainRuneAt(p gruid.Point) rune {
