...
* The Orb may now interfere with teleportation about half of the time, driving
  ...
  (see issue #11).
* New early guardian: **chaos megabat**. It features a sneaky attack pattern,
  like Vampiric Bat players, but also a chaos bite that inflicts random
  effects (often good ones, sometimes bad ones). It appears as a totem or
  portal guardian early on on most runs, usually around levels 2-3. Not strong
  by itself for a lone guardian, but somewhat durable, unpredictable and
  difficult to avoid.
* New portal guardian: **dragging alligator**. It features a dragging attack
  pattern, like Spinning Crocodile players, as well as a confusing bite, easily
  leading to the Confusion+Imbalance “drunken fight” defensive effect. It
  alternates with blazing golem as a portal guardian, but a bit less
  frequently. It has higher attack but more modest defenses.
...
* Zebra's Disorient status now lasts an extra turn, for better balance.
* Elephant's “stomp” ability now makes you berserk for 4 turns, instead of 5,
  ...
  wall-destroying effects.
* Lion's roar now inflicts 4-5 turns of Fear, instead of 5, with 4 being more
  likely on close targets. This change helps balance hydra+lion and
...
* Cat's dazing effect on swapping happens now less often than before; it's also
  somewhat less likely but longer on average at long distances to make distant
  swapping by monster cats more unlikely to be undone just afterwards.
...
UI regression when examining monsters on top of items.

* Lignification and Daze now show as `Lig*` and `Daz*` for players with “woody
  legs” or “daze resistance” respectively (see comments in issue #15 for
  context). That should make it easier for the player to recall that those
  statuses have looser constraints in those special cases. Status descriptions
  have been updated to explain this. Also, descriptions on mouse hover are now
  the same detailed ones that appear in the help, and not the shortened ones
  that appear in ability and comestible descriptions.
* Lignification no longer gives extra defense against ranged attacks, as the
  other recent buffs are much better, scales already did the same, and wind fox
  against tree balance is less important now, in particular with advanced
  spirits (see issue #15 for context). That simplifies Lignification a bit, as
  its description was becoming quite long.
* Lignified monsters are no longer moved by Wind Fox's “pushing gale” (see
  issue #15). In particular, walking trees aren't moved by pushing gale
  anymore.
* Fix regression in examine mode that would prevent showing more than one
  entity description box at once (see issue #15). The regression was introduced
  when the new box scrolling improvements from Harmonist were adapted to
  Shamogu.
* Properly support mouse wheel scrolling in browser version.

# v1.4.0 2026-02-01

This release is a maintenance one with a few minor fixes and UI improvements.

* Improve coloring during last turn of player status effects in the status bar:
  now the color is different depending on whether the status effect is relevant
  during the player's turn, the monsters', or both. In particular, Disorient
  now shows in grey when only one turn remains, which makes it clearer that it
  has no effect on the last player's turn (see issue #13 for context).
* Temporal Cat swapping with Runic Chicken on top of a trap now makes the
  target swapped monster trigger the trap (see issue #13).
* Mention in help that cat attacks swap clouds, but not terrain.
* Native versions: make *path* in “Full game statistics at *path*” abbreviate
  the game’s data directory to avoid mentioning the username, so that replay
  files don’t contain any personal data and can be shared anonymously if
  desired. The browser version didn't have this issue, as it just says “Full
  game statistics written below”.
* ASCII versions: update cloud letter to `≡` instead of `§`, as it matches
  better the tiles version and mostly looks like the standard symbol for fog.
  Compatibility is preserved for replays of old games, as both characters
  render to the same tile.
* More flexible target description scrolling (with mouse or page up/down). Each
  scroll down now removes one box, eventually removing all of them. Scrolling
  up restores all boxes. This is useful if one wants to see only the map while
  targeting, for example when doing long travel and wanting to see the
  highlighted path without any description boxes hiding part of it.
* Record a few extra statistics (number of waits, times you felt cornered,
  jumpings off walls, passings through translucent walls).
* A couple of improvements in singular/plural handling with a prefix count in
  text (like “for 1 turn” instead of “for 1 turns” in status description on
  mouse hover).
* When attacked by a rat while lignified with the Stomping Elephant, the player
  now becomes Berserk (like when afraid).
* Fix mod conflict issue between “Gluttony Rework” and “Small Inventory” mods
  when choosing first comestible from the inventory and second comestible from
  the ground (see issue #14). Also, ensure that comestible description is
  immediately updated to reflect selected second comestible after choosing the
  first one.
* Fix minor error in label size computation of status description on mouse
  hover in some edge cases (that maybe did not actually happen in Shamogu
  in practice, as this issue was caught for similar code in Harmonist).

# v1.3.0 2026-01-08

This release is a maintenance one with a few small fixes, mostly about UI, and
updated backend dependencies.

* Require Go 1.24 or later and update gruid-tcell, gruid-sdl and gruid-js
  dependencies.
* Improve end-of-game summary and fix its title in the pager. The summary now
  includes the detailed information about mods and spirits, as there's always
  enough screen space for those without need for scrolling, and it's nice to
  see them without having to look at the full game statistics (for example when
  sharing a screenshot or only the replay instead of the full dump).
* Vampiric Bat has now +2 charges at level 5 if the No Recharges mod is
  enabled, instead of +1. This was an oversight (see issue #11).
* SDL2 version (gruid-sdl): the old bindings were replaced by new dedicated
  ones that are simpler and compile much faster. The `-noaccel` option was
  removed, making accelerated build the default and only option, which is
  hopefully ok nowadays (the software rendering mode wasn't really tested
  properly anyway).
* SDL2 version (gruid-sdl): fix coordinate issue with mouse in fullscreen, due
  to scaling not being updated correctly (possibly a regression from
  enabling accelerated rendering in the previous release).
* README: updated SDL2 build instructions (explicitly mention we need SDL2 and
  not SDL3, and update link to the SDL2 installation page directly).
* Browser version (gruid-js): fixed Backspace key reading (usable in the pager
  as an alternative to `u` for half-page up).
* A couple of minor log message fixes and improvements.
...
* New experimental challenge mod: **Healing Combat**. Healing happens through
  ...
  lacks polish with respect to the default experience. It will be reworked in a
  future version.
...
* Afraid monsters are now somewhat biased toward going in the opposite
  direction from the player, instead of fleeing laterally, so that non-Hydra
  players get more offensive use from Fear.

UI, graphics, QoL:

* SDL2 version: graphic acceleration is now enabled by default and there is a
  new `-noaccel` flag for disabling it.
* SDL2 version: new command-line options `-w` and `-h` to set window width and
  ...
  factors. At the very least, using multiples of 0.25 seems better with
  software rendering when `-noaccel` is provided (some other values result in
  visual glitches).
